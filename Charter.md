# Arche Kernel Charter (Final – Public Version)
...
- Critical paths must be deterministic, bounded, and constant-time
...
- Manifest parsing + ed25519 signature verification (constant-time)
- Hardcoded plugin directory loading with bounded iteration
- Bounded execution via epoch interruption (higher layer ticking)
- Stateless, fail-fast loader (no lingering state)

Current implementation characteristics:
- No host imports in WIT (stdio fully sealed – instantiation trap on leaks)
- Hardcoded bounds (MAX_PLUGINS, MAX_DIR_ENTRIES)
